...
\end{equation} 
We have summarized a function {\tt DualAveragingStepSizeAdaptation} which will work with both samplers in algorithm \ref{algo:dual_step_size_adaptation}.

\begin{figure}[H]
	\begin{algorithm}[H]
	\caption{Dual Averaging Step Size Adaptation}\label{algo:dual_step_size_adaptation}
	\begin{algorithmic}
        \Function{{\tt DualAveragingStepSizeAdaptation}}{$H_t^S, \delta, t, t_0, \bar{x}_t, k$}
            ...
            \Else
                \State $h \gets (t-1)h + H_t$ \Comment{Aggregate the average statistic}
                \State $x_{t+1} \gets \mu - \frac{\sqrt{t}}{\gamma(t + t_0)}th$ \Comment{First part of update rule in eq.~\eqref{eq:update_rule}}
