...
\begin{figure}[H]
  \begin{algorithm}[H]
    ...
    \begin{algorithmic}
      ...
      \If {$p \geq u$}
        \State $\theta \to \theta'$ \Comment{Accept transition}
      \Else
        \State $\theta \to \theta$  \Comment{Reject transition}
      \EndIf
    \end{algorithmic}
  \end{algorithm}
\end{figure}
...
\begin{figure}[H]
	\begin{algorithm}[H]
		...
		\begin{algorithmic}
			\State 1. $\hat{p}_i(t + h/2) = \hat{p}_i(t) - \frac{h}{2}\pdv*{V(\hat{q}(t))}{q_i} $\\
			\State 2. $\hat{q_i}(t+h) = \hat{q}_i(t) + \frac{h}{m_i}\hat{p}_i(t+h/2)$\\
			\State 3. $\hat{p_i}(t+h) = \hat{p}_i(t + h/2) -  \frac{h}{2}\pdv*{V(\hat{q}(t+h))}{q_i}$
		\end{algorithmic}
	\end{algorithm}
\end{figure}
